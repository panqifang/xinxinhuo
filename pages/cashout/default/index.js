...
import { getCashOutInfo, checkSmsCode, cashOut, getSmsCode } from '../../../script/api'
...
Page(Object.assign({}, Toast, {
    ...
    // 获取数据
    getData: function () {
        getCashOutInfo({ shopid: self.data.shopid }).then(data => {
            wx.hideLoading()
    ...
    cashSubmit: function (e) {
        ...
        // 判读是否输入金额
        if (isMoneyNum(formValue.cash) <= 0) {
            return self.showZanToast('清输入提现金额')
        }
        // 判断提现金额
        let minmoney = parseFloat(self.data.shopInfo.minmoney)
        let maxmoney = parseFloat(self.data.shopInfo.maxmoney)
        let cash = parseFloat(self.data.shopInfo.cash)
        // 不能小于最新提现金额
        if (checkTwoFloat(minmoney, formValue.cash)) {
            return self.showZanToast('最小提现金额为' + minmoney + '元')
        }
        // 不能大于提现金额
        if (checkTwoFloat(formValue.cash, cash)) {
            return self.showZanToast('余额不足提现')
        }
        // 不能大于提现金额
        if (checkTwoFloat(formValue.cash, maxmoney)) {
            return self.showZanToast('最大提现金额为' + maxmoney + '元')
        }
                ...
                wx.showModal({
                    ...
                    success: function (res) {
                        if (res.confirm) {
                            ...
                            cashOut({ mobile: formValue.mobile, realname: formValue.card_name, card: formValue.card, cash: formValue.cash, bank_id: self.data.bankid, shopid: self.data.shopid }).then(data => {
                                console.log(data)
                                wx.showToast({
                                    title: '提现申请成功提交',
                                    icon: 'success',
